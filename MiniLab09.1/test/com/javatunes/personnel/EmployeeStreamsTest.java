...
public class EmployeeStreamsTest {
    private Collection<Employee> allEmployees; // <T> = Employee
    ...
    public void findAll_nameLengthAtMost5_sortBySalary() {
        List<Employee> result = allEmployees.stream()
                .filter(emp -> emp.getName().length() <= 5) // predicate
//                .sorted((emp1, emp2) -> emp1.getSalary().compareTo(emp2.getSalary())
//                .sorted(Comparator.comparing(emp -> emp.getSalary())) //comparing - key extractor; comparator;
                .sorted(comparing(Employee::getSalary)) //would be Comparartor.comparing, but w/ import, dont need it
                .collect(toList());
        dump(result);
    }
...
//                .sorted( (emp1, emp2) -> emp1.getHireDate().compareTo(emp2.getHireDate()))
    ...
    public void threeHighestPaid_sortByName() {
        List<Employee> result = allEmployees.stream()
//                .sorted( (emp1, emp2) -> emp2.getSalary().compareTo(emp1.getSalary())) // sorted salary descending, flip the emp1 & emp2
                .sorted(comparing(Employee::getSalary).reversed() ) // sorted salary descending, flip the emp1 & emp2
                .limit(3)
//                .sorted( (emp1, emp2) -> emp1.getName().compareTo(emp2.getName()))
                .sorted(comparing(Employee::getName))
                .collect(toList()); // would be Collectors.toList() but w/ import, dont need it
        dump(result);
    }
...
//                .map(emp -> emp.getName())
                .map(Employee::getName)// gets the names only as a string (Stream<String>)
                .sorted()
                .collect(toList());
        dump(names);
    }
    ...
    public void payAll_salaryAtMost50000_lowestPaidFirst() {
        allEmployees.stream()
                ...
                .forEach(Employee::pay); //do stuff - pay them
    }
    ...
    public void workAll_salaryAtLeast75000_descendingSalaryOrder() {
        allEmployees.stream()
                ...
                .forEach(Employee::work); // do stuff - make them work
    }
    ...
    public void giveRaise_salaryLessThan50000_hiredAfter2000_lowestPaidFirst() {
        List<Employee> result = allEmployees.stream()
                ...
                .sorted(comparing(Employee::getSalary))
                .peek(emp -> emp.setSalary(100_000.0)) // emp in, do something to it
                .collect(toList());
        dump(result);
    }
    ...
    public void twoGroups_atLeast50K_lessThan50K() {
        Map<Boolean, List<Employee>> map = allEmployees.stream()
                .collect(partitioningBy(emp -> emp.getSalary() >= 50000.0));
        dump(map);
    }
    ...
    public void groupingByYearOfHire() {
        Map<Integer,List<Employee>> map = allEmployees.stream()
                .filter(emp -> emp.getHireDate().getYear() == 1999 | emp.getHireDate().getYear() == 2009)
    ...
    private static void dump(Collection<?> collection) {
        collection.forEach(System.out::println); // emp -> sout(emp)
    }
